...
// try ~ catch 예외처리 방법
	public static void main(String[] args) {
		 ...
		 * 			예외 발생이 예외되는 수행문;
		 * 			예외 발생이 예외되는 수행문;
		 * 		} catch (예외클래스명1 변수명1) { 자식예외는 위쪽 부모예외는 아래쪽
		 * 			예외처리 수행문1;
		 * 			예외처리 수행문2;
		 * 		} catch (예외클래스명2 변수명2) {
		 * 			예외처리 수행문2;
		 * 			예외처리 수행문2;
		 * 		} catch (Exception 변수명3) { // 보험이라고 생각하자.
		 * 			예외처리수행문3;
		 * 			예외처리수행문3;
		 * 		}
		 * 
		 ...
		 * 		  세번재 catch 블록에서 해당 예외를 가로채고 변수명3에 대입한 다음,
		 * 		  예외처리수행문3을 실행한다.	
		 */
		
		// Checked Exception이 발생하는 수행문 실행하고, 예외처리하기
		try {
			FileWriter writer = new FileWriter("src/sample01/sample01.txt"); // IOException 예외발생
			writer.write("예외처리 실습해보기"); // 예외발생 수행문;
			writer.close();
